...
String nomModule = "InterBouton";                 // Nom usuel de ce module. Sera visible uniquement dans les Log Domoticz.
...
void SendData () {
  ...
  // Suivant le résultat lu, Création de la commande qui sera envoyé via MQTT.
  if (buttonEtat == 1) {
    // Attention, nous sommes connecté en Pull-Up, il faut donc inverser l'ordre reçu : 1 = repos.
    root["switchcmd"] = "Off";
  } else {
    root["switchcmd"] = "On";
  }
